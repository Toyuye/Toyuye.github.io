...
const throttle = function (fn: any, interval: number, _this: any = window) {
    let _self = fn, timer: any, firstTimer: boolean = true;
    return () => {
        var args: IArguments = arguments, _me: any = _this;
        if(firstTimer) {
...
const _storage = () => {
    let testKey: string = '_LOCALSTORAGETEST', storage: any = window.localStorage
    try {
